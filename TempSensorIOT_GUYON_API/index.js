...
app.get('/capteurs', async (req, res) => {
  ...
  app.put('/capteurs/:id', async (req, res) => {
    ...
    try {
      let capteur = await Capteur.findOne({ nom: id });
      if (!capteur) {
        capteur = new Capteur({
          nom: id,
        ...
        await capteur.save();
        return res.json({ message: 'Les données du capteur ont été créées avec succès' });
      }
      capteur.temperature = temperature;
      await capteur.save();
      res.json({ message: 'Les données du capteur ont été mises à jour avec succès' });
    } catch (err) {
      res.status(500).json({ message: `Une erreur est survenue lors de la mise à jour du capteur ${err}` });
    }
  });
